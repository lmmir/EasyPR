...
namespace easypr {
	...
	bool CPlateLocate::verifySizes(RotatedRect mr) {
		...
		//偏差率
		//https://www.cnblogs.com/subconscious/p/4047960.html
	...
	*/
	...
	int CPlateLocate::sobelFrtSearch(const Mat &src,
		...
		while (itc != contours.end()) {
			RotatedRect mr = minAreaRect(Mat(*itc));//获取包裹轮廓的最小矩形
			{
				//Mat image(200, 200, CV_8UC3, Scalar(0));
				//rectangle(image, mr.boundingRect(), Scalar(255, 0, 0));
				//SHOW_IMAGE(image, 1);
			}
			//长宽比+面积判断轮廓是否符合车牌
			if (verifySizes(mr)) {
